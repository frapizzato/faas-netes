...
// NewHandlerFunc_TLS creates a TLS http.HandlerFunc to proxy function requests.
// When verbose is set to true, the timing of each invocation will be printed out to
// stderr.
// The returned http.HandlerFunc will ensure:
//
...
//
// Note that this will panic if `resolver` is nil.
func NewHandlerFunc_TLS(config types.FaaSConfig, resolver BaseURLResolver, verbose bool) http.HandlerFunc {
	if resolver == nil {
		panic("NewHandlerFunc: empty proxy handler resolver, cannot be nil")
	}

	proxyClient := NewProxyClientFromConfig_TLS(config)
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodPost,
			...
			http.MethodOptions,
			http.MethodHead:
			proxyRequest_TLS(w, r, proxyClient, resolver, &reverseProxy, verbose)
...
func NewProxyClientFromConfig_TLS(config types.FaaSConfig) *http.Client {
	return NewProxyClient_TLS(config.GetReadTimeout(), config.GetMaxIdleConns(), config.GetMaxIdleConnsPerHost())
}
...
func NewProxyClient_TLS(timeout time.Duration, maxIdleConns int, maxIdleConnsPerHost int) *http.Client {
	return &http.Client{
...
func proxyRequest_TLS(w http.ResponseWriter, originalReq *http.Request, proxyClient *http.Client, resolver BaseURLResolver, reverseProxy *httputil.ReverseProxy, verbose bool) {
	...
	proxyReq, err := buildProxyRequest_TLS(originalReq, functionAddr, pathVars["params"])
	if err != nil {
...
func buildProxyRequest_TLS(originalReq *http.Request, baseURL url.URL, extraPath string) (*http.Request, error) {
	...
	url := url.URL{
		Scheme:   "https", // MODIFIED: Force HTTPS for TLS
		Host:     host,
		Path:     extraPath,
